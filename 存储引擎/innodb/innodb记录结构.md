...
![image-20190828182020191](assets/innodb记录结构/image-20190828182020191.png)
...
![image-20190828182354744](assets/innodb记录结构/image-20190828182354744.png)
...
![image-20190828182520418](assets/innodb记录结构/image-20190828182520418.png)
...
![image-20190828182621676](assets/innodb记录结构/image-20190828182621676.png)
...
![image-20190828182702685](assets/innodb记录结构/image-20190828182702685.png)
...
![image-20190828182727601](assets/innodb记录结构/image-20190828182727601.png)

所以第一条记录的`NULL值列表`用十六进制表示就是：`0x00`。

- 对于第二条记录来说，`c1`、`c3`、`c4`这3个列中`c3`和`c4`的值都为`NULL`，所以这3个列对应的二进制位的情况就是：

![image-20190828182753088](assets/innodb记录结构/image-20190828182753088.png)

- 所以第二条记录的`NULL值列表`用十六进制表示就是：`0x06`。

所以这两条记录在填充了`NULL值列表`后的示意图就是这样：

![image-20190828182807874](assets/innodb记录结构/image-20190828182807874.png)

##### 记录头信息

除了`变长字段长度列表`、`NULL值列表`之外，还有一个用于描述记录的`记录头信息`，它是由固定的`5`个字节组成。`5`个字节也就是`40`个二进制位，不同的位代表不同的意思，如图：

![image-20190828182829713](assets/innodb记录结构/image-20190828182829713.png)
...
![image-20190828182859178](assets/innodb记录结构/image-20190828182859178.png)
...
![img](assets/innodb记录结构/169710e973b70372)
...
![img](assets/innodb记录结构/169710e9aab47ea5)
...
![1567312734593](assets/innodb记录结构/1567312734593.png)
...
3. `InnoDB`目前定义了4种行格式
    ...
    - COMPACT行格式
        ...
        具体组成如图：
        ![1567312796397](assets/innodb记录结构/1567312796397.png)

    - Redundant行格式
      
        具体组成如图：
        ![1567312808019](assets/innodb记录结构/1567312808019.png)
