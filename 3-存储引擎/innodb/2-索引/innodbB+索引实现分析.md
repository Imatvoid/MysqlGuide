...
![img](assets/innodbB+索引实现分析/16a01bd1b8eafbb4)

其中页a、页b、页c ... 页n 这些页可以不在物理结构上相连，只要通过双向链表相关联即可。

## 无索引查找

为了方便大家理解，我们下边先只唠叨搜索条件为对某个列精确匹配的情况，所谓精确匹配，就是搜索条件中用等于`=`连接起的表达式，比如这样：
...
### 在很多页中查找因为页面类型前边都有个`FIL_PAGE`或者`FIL_PAGE_TYPE`的前缀，为简便起见我们后边唠叨页面类型的时候就把这些前缀省略掉了，比方说`FIL_PAGE_TYPE_ALLOCATED`类型称为`ALLOCATED`类型，`FIL_PAGE_INDEX`类型称为`INDEX`类型。
...
![img](assets/innodbB+索引实现分析/16a01bd1ba8d05b2)
...
![](assets/innodbB+索引实现分析/16a01bd1b0cad198)



把一些记录放到页里边的示意图就是：

![](assets/innodbB+索引实现分析/16a01bd1be0d43ce)
...
![](assets/innodbB+索引实现分析/16a01bd27e1f3cf3)
...
![](assets/innodbB+索引实现分析/16a01bd282d6b9b9)
...
设计`InnoDB`的大叔们需要一种可以灵活管理所有`目录项`的方式。他们灵光乍现，忽然发现这些`目录项`其实长得跟我们的用户记录差不多，只不过`目录项`中的两个列是`主键`和`页号`而已，所以他们<span style="color:red">复用了之前存储用户记录的数据页来存储目录项，为了和用户记录做一下区分，我们把这些用来表示目录项的记录称为`目录项记录`</span>。那`InnoDB`怎么区分一条记录是普通的`用户记录`还是`目录项记录`呢？别忘了记录头信息里的`record_type`属性，它的各个取值代表的意思如下：
...
![](https://user-gold-cdn.xitu.io/2019/4/9/16a01bd295fd42b5?w=1178&h=535&f=png&s=149447)
...
![](assets/innodbB+索引实现分析/16a01bd29ebc7a4c)
...
![](assets/innodbB+索引实现分析/16a01bd2a6c7a65f)
...
![](assets/innodbB+索引实现分析/16a01bd2a6fb9126)
...
![](assets/innodbB+索引实现分析/16a01bd2a89adfa5)
...
![](assets/innodbB+索引实现分析/16a01bd2b0b70d72)
...
![](assets/innodbB+索引实现分析/16a01bd2c92fbca0)
...
我们知道`InnoDB`中<span style="color:red">索引即数据，也就是聚簇索引的那棵`B+`树的叶子节点中已经把所有完整的用户记录都包含了</span>，而`MyISAM`的索引方案虽然也使用树形结构，但是却将索引和数据分开存储：
...
![](assets/innodbB+索引实现分析/16a01bd2d0c5ad53)
