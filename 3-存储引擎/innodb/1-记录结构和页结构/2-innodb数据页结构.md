...
`页`是`InnoDB`管理存储空间的基本单位，一个页的大小一般是`16KB`。`InnoDB`为了不同的目的而设计了许多种不同类型的`页`，比如存放`表空间头部信息`的页，存放`Insert Buffer`信息的页，存放`INODE`信息的页，存放`undo`日志信息的页,存放表中的记录的`数据页`等.

## 数据页结构的快速浏览

数据页代表的这块`16KB`大小的存储空间可以被划分为多个部分，不同部分有不同的功能，各个部分如图所示：

![1567315250559](assets/2-innodb数据页结构/1567315250559.png)
...
![img](assets/2-innodb数据页结构/16a3e5782ca4a346)
...
![img](assets/2-innodb数据页结构/16a3e5782c944eaf)
...
|   `heap_no`    |       `13`        |    表示当前记录在页中的index,从0开始,最小记录0/最大记录1     |
...
![img](assets/2-innodb数据页结构/16a3e5782fc0ef6a)
...
`InnoDB`自动给每个页里边儿加了两个记录，由于这两个记录并不是我们自己插入的，所以有时候也称为`伪记录`或者`虚拟记录`。这两个伪记录一个代表`最小记录`，一个代表`最大记录`.所谓最小和最大是相对于主键的大小排序.

这两条记录的构造十分简单，都是由5字节大小的`记录头信息`和8字节大小的一个固定的部分组成的，如图所示

![img](assets/2-innodb数据页结构/16a3e5783051dd99)
...
![image-20191027193533720](assets/2-innodb数据页结构/image-20191027193533720.png)
...
> next_record这个指针指向记录头信息和真实数据之间的位置
>
> 这个位置刚刚好，向左读取就是记录头信息，向右读取就是真实数据。我们前边还说过变长字段长度列表、NULL值列表中的信息都是逆序存放，这样可以使记录中位置靠前的字段和它们对应的字段长度信息在内存中的距离更近，可能会提高高速缓存的命中率。



如果我们再把第二条记录的插入会去,innodb不会为它申请新的存储空间，而是直接复用了原来被删除记录的存储空间。
...
最笨的办法：从`Infimum`记录（最小记录）开始，沿着链表一直往后找.当然这样太慢了
...
![img](assets/2-innodb数据页结构/16a3e5790b3e26c2)
...
![img](https://user-gold-cdn.xitu.io/2019/4/21/16a3e57920d232ea?w=994&h=550&f=png&s=103256)
...
![img](assets/2-innodb数据页结构/16a3e57926e95943)
...
- `FIL_PAGE_PREV`和`FIL_PAGE_NEXT`
  ...
  ![img](assets/2-innodb数据页结构/16a3e579271b492f)
